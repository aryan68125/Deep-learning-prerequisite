...
print("Using Logisitc Regression from scikitLearn")
...
print("Using my own Logistic Regression model using numpy with L1 regularization")
...
'''
W = np.random.randn(D) / np.sqrt(D) #weights setting up to be at random
...
for i in range(20000):
    ...
    #Applying L1 regularization on the cost , error or objective function
    ctrain = cross_entropy(Ytrain, pYtrain)+ l1*np.abs(W).mean()
    #calculate the test cost
    #Applying L1 regularization on the cost , error or objective function
    ctest = cross_entropy(Ytest,pYtest)+ l1*np.abs(W).mean()
    ...
    #Applying L1 regularization on gradient descent equation for weight calculation
    W -= learning_rate*(Xtrain.T.dot(pYtrain - Ytrain) + l1*np.sign(W)) # defining the weights via gradient descent equation
